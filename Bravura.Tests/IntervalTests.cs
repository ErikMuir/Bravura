...
namespace Bravura.Tests
{
    public class IntervalTests
    {
        ...
        [Fact]
        public void Interval_Equality_Test()
        {
            var fakeInterval = new Interval(0, Accidentals.Natural, 1, "Tonal Center", "");
            Assert.True(fakeInterval == Intervals.PerfectUnison);
            Assert.True(fakeInterval.Equals(Intervals.PerfectUnison));
            Assert.Equal(fakeInterval.GetHashCode(), Intervals.PerfectUnison.GetHashCode());
        }
    }
}